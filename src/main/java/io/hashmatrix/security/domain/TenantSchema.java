 ...
 *
 * <p>由租户标识派生 {@code sec_<sanitized>}：仅保留 {@code [a-z0-9_]}、其余折叠为 {@code _}，
 * 确保可安全用于 {@code SET search_path}（杜绝注入）。安全资产（分级/标签/审批）以此为隔离边界，
 * 绝不跨租户串。
 *
 * @param name 归一化后的 schema 名，形如 {@code sec_acme}
 */
public record TenantSchema(String name) {

    /** schema 前缀：标识安全分系统的租户库。 */
