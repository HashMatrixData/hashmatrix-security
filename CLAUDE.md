# CLAUDE.md — hashmatrix-security 协作与合规指引
...
**本仓视角（security）**：分类分级 / 标签 / 审批 / 审计**以租户为边界**——权限在租户内（路由级 + 按钮级，映射 Keycloak 角色）；多租户数据隔离是安全底线，任何查询默认带租户约束。
...
数据安全分系统：分类分级、标签规则、工作流审批、安全审计、资源监控。
